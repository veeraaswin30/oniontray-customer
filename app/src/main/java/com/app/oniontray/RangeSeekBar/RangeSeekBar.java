...
package com.app.oniontray.RangeSeekBar;
...
public class RangeSeekBar<T extends Number> extends ImageView {
    ...
    private String currencySympool = "₹";
    ...
    private final Bitmap thumbImage = BitmapFactory.decodeResource(getResources(), R.drawable.ic_filter_prize_round_ic);
    private final Bitmap thumbPressedImage = BitmapFactory.decodeResource(getResources(),
            R.drawable.ic_filter_prize_round_ic);
     ...
     */
    public static final int DEFAULT_COLOR = Color.argb(255, 221, 0, 20);
    /**
    ...
    public RangeSeekBar(Context context) {
        ...
        prefManager = new LoginPrefManager(context);

        currencySympool = prefManager.getCurrencySymbole();

        Log.e("currencySympool", "-" + currencySympool);
...
//            String minText = prefManager.getFormatCurrencyValue("" + getSelectedMinValue());
//            String maxText = prefManager.getFormatCurrencyValue("" + getSelectedMaxValue());

            String minText = "" + currencySympool + " " + String.valueOf(getSelectedMinValue());
            String maxText = "" + currencySympool + " " + String.valueOf(getSelectedMaxValue());

            float minTextWidth = paint.measureText(minText) + offset;
            float maxTextWidth = paint.measureText(maxText) + offset;

            if (!mSingleThumb) {
                canvas.drawText(minText, normalizedToScreen(normalizedMinValue) - minTextWidth * 0.5f,
                        mDistanceToTop + mTextSize, paint);
            }

            canvas.drawText(maxText, normalizedToScreen(normalizedMaxValue) - maxTextWidth * 0.5f,
                    mDistanceToTop + mTextSize, paint);
        }
