...
import com.app.oniontray.Adapters.MyWalletCreditAdapter;
...
public class MyWalletCreditFragment extends Fragment {
    ...
    private MyWalletCreditAdapter myWalletCreditAdapter;

    private TextView my_wal_cred_debi_embty_msg_txt_view;


    public MyWalletCreditFragment() {
    }
...
//                        Log.e("UserWalletRequest", "" + response.raw().toString());
                        ...
                        if (response.body().getResponse().getHttpCode() == 200) {
                            ...
                            if (response.body().getResponse().getCreditWalletList().size() != 0) {

                                myWalletCreditAdapter = new MyWalletCreditAdapter(context, (ArrayList<CreditWalletData>) response.body().getResponse().getCreditWalletList());
                                my_wal_cred_debi_recy_view.setAdapter(myWalletCreditAdapter);
                                ...
                                my_wal_cred_debi_embty_msg_txt_view.setText("" + context.getString(R.string.my_wallet_credit_err_msg_txt));
                            }

                            if (myWallCreditFragInterface != null) {
                                myWallCreditFragInterface.MyWalletCreditFragment("" + response.body().getResponse().getUserWalletAmount());
                            }
...
//            Log.e("UserWalletRequestMethod", "Exception :" + e.getMessage());
    ...
    public MyWallCreditFragInterface myWallCreditFragInterface;

    public void CallMyWalletCreditFragInterface(MyWallCreditFragInterface myWallCreditFragInterface) {
        this.myWallCreditFragInterface = myWallCreditFragInterface;
    }

    public interface MyWallCreditFragInterface {
        void MyWalletCreditFragment(String wallet_amt);
    }
